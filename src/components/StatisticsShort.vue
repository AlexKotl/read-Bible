<template>
  <div style="position: relative">
    <div style="display:flex">
      ...
      <div class="right">
        {{ $t("Readed") }}:
        <div class="number">{{ this.totalPercents }}%</div>
        {{ $t("ReadedChapters") }}:
        <div class="number">
        ...
        {{ $t("Achievements") }}:
        <div class="number">
          {{ this.stats.achievements_count }} <small>{{ $t("outOf") }}</small>
    ...
    <div class="achievements">
      <router-link :to="{ name: 'achievements' }">
        <span
          ...
          :key="'ach_' + achievement.id"
        >
          <img
            :src="
              require('../assets/achievements/' +
                achievement.name +
                (achievement.is_done == 1 ? '_done' : '') +
                '.png')
            "
...
export default {
  ...
  computed: {
    ...
    totalPercents: function() {
      const perc =
        Math.round((this.stats.read_chars / this.stats.total_chars) * 1000) /
        10;
  ...
  methods: {
    async init() {
      ...
      let labels = [];
      for (let i = 30; i >= 0; i--) {
        const date = new Date(new Date() - 60 * 60 * 24 * 1000 * i)
          .toISOString()
          .substr(0, 10);
        const dayData =
          this.stats.by_month[date] !== undefined
            ? this.stats.by_month[date].chars
            : 0;
        monthData.push(dayData);
        labels.push(date);
        this.monthChartTooltip[date] =
          this.stats.by_month[date] !== undefined
            ? this.stats.by_month[date].chapters
            : 0;
      }
      this.monthChartData = {
        labels: labels,
