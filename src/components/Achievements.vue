<template>
  <div>
    <Achievement
      v-if="show_achievement"
      :title="current_title"
      :icon="current_icon"
      @click.native="show_achievement = false"
    ></Achievement>
    ...
    <div class="achievements-list">
      <div
        ...
        class="item"
        @click="
          achievement.is_done == 1
            ? showAchievement(achievement.title, achievement.name)
            : ''
        "
      >
...
export default {
  ...
  async created() {
    ...
    data.map((val) => {
      var img;
      try {
        img = require("../assets/achievements/" +
          val.name +
          (val.is_done == 1 ? "_done" : "") +
          ".png");
      } catch (e) {
