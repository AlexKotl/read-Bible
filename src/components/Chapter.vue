<template>
  <div>
    ...
    <h3 v-else>{{ $t("Chapter") }} {{ chapter.number }}</h3>
    ...
    <div class="chapter-footer">
      <div v-if="!getUser.session_id">
        <router-link :to="{ name: 'login' }" class="button disabled"
          >{{ $t("MarkChapterAsRead") }} {{ getUser.session_id }}</router-link
        >
        <br /><small
          >{{ $t("YouNeed") }}
          ...
          >, {{ $t("toMarkChapterAsRead") }}</small
        >
      </div>
      <div v-else-if="isRead">
        <LoadingButton
          class="button green"
          @click.native="markRead"
          :loading="isLoading"
        >
      ...
      <br /><br />
      <router-link
        :to="{ name: 'chapter', params: { id: chapter.prev_id } }"
        v-if="chapter.prev_id"
      >
        &laquo; {{ $t("Previous") }}
      </router-link>
      <router-link
        :to="{ name: 'chapters' }"
        style="font-size: 30px; margin: 0 20px; text-decoration:none"
      >
        &#8962;
      </router-link>
      <router-link
        :to="{ name: 'chapter', params: { id: chapter.next_id } }"
        v-if="chapter.next_id"
      >
        {{ $t("Next") }} &raquo;
      </router-link>
    </div>
  ...
  computed: {
    ...
    isRead() {
      ...
      Object.values(this.getChapters).forEach((book) => {
        let found = book.find(
          (chapter) => chapter.id == this.id && chapter.is_read == 1
        );
        if (found !== undefined) {
          is_read = true;
        }
